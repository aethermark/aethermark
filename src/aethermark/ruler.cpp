...
namespace aethermark
{
    ...
    template <typename T>
    void Ruler<T>::at(const std::string &name, T fn, types::RuleOptions options)
    {
    ...
    template <typename T>
    void Ruler<T>::before(const std::string &beforeName, const std::string &ruleName, T fn, types::RuleOptions options)
    {
        int idx = findRuleIndex(beforeName);
        if (idx < 0)
            throw std::runtime_error("Rule not found: " + beforeName);
        rules.insert(rules.begin() + idx, types::Rule<T>{ruleName, true, fn, options.alt});
        cacheDirty = true;
    }

    template <typename T>
    void Ruler<T>::after(const std::string &afterName, const std::string &ruleName, T fn, types::RuleOptions options)
    {
        int idx = findRuleIndex(afterName);
        if (idx < 0)
            throw std::runtime_error("Rule not found: " + afterName);
        rules.insert(rules.begin() + idx + 1, types::Rule<T>{ruleName, true, fn, options.alt});
        cacheDirty = true;
    }

    template <typename T>
    void Ruler<T>::push(const std::string &ruleName, T fn, types::RuleOptions options)
    {
        rules.push_back(types::Rule<T>{ruleName, true, fn, options.alt});
        cacheDirty = true;
    }
